...
"""特定分类采集模块 - 从 abc123 和 iptv-org 体育专类源采集指定类别"""
...
def parse_iptvorg_sports_m3u(content: str) -> List[Tuple[str, str]]:
    """
    解析 iptv-org 体育专类 M3U 内容，提取所有频道
    返回 [(频道名, URL), ...]
    """
    if not content:
        return []
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(source_url, timeout=15, headers=headers) as resp:
                if resp.status != 200:
                    logger.warning(f"⚠️ iptv-org 体育专类源返回 HTTP {resp.status}")
                    return []
                content = await resp.text()
                return parse_iptvorg_sports_m3u(content)
    except Exception as e:
        logger.error(f"❌ 获取 iptv-org 体育专类频道失败: {e}")
        return []
...
async def collect_and_append_special_categories(output_dir: Path, db=None) -> Dict[str, int]:
    """
    主函数：从 abc123 和 iptv-org 体育专类源采集指定分类并追加到输出文件
    """
    logger.info("🧠 开始智能补充采集（从 abc123 + iptv-org 体育专类源）...")

    # 1. 从 abc123 获取所有目标分类
    abc123_data = await fetch_abc123_source()
    
    # 2. 从 iptv-org 体育专类源获取体育频道
